...
contract('KPS', accounts => {
  ...
  const GAS_PRICE = BigInt(20000000000); // This is the default for truffle test network
  ...
  async function createPlayers([selection0, selection1]) {
    const players = [
      { address: accounts[3], selection: selection0, nonce: 12345, expectedBalance: 0 },
      { address: accounts[4], selection: selection1, nonce: 67890, expectedBalance: 0 },
    ];
    for (const player of players) {
      player.expectedBalance = BigInt(await web3.eth.getBalance(player.address));
    }
    return players;
  }
  ...
  it('should play full fair game', async () => {
    ...
    for (const player of players) {
      ...
      player.expectedBalance -= BigInt(receipt.gasUsed) * GAS_PRICE;
    }

    const [ gameStartedEvent ] = await instance.getPastEvents('GameStarted');
    const gameIdentifier = gameStartedEvent.returnValues.gameIdentifier;
    ...
    for (const player of players) {
      const accountBalance = BigInt(await web3.eth.getBalance(player.address));
